...
from app.model.video import VideoType,FromType,NationalityType,IdentityType,Video,VideoSub,VideoStart
from app.untils.common import check_and_video_type,handle_video
...
class ExternaVideo(View):
    ...
    def get(self,request):
        ...
        ex_videos=Video.objects.exclude(from_to=FromType.custom.value)#拿除了自制以外的视频
    ...
    def post(self,request):
        ...
        if not all([name,image,video_type,from_to,nationality,info]):
            return redirect('{}?error={}'.format(reverse('reverse_path'),"缺少必要字段"))

        result=check_and_video_type(VideoType,video_type,'非法的视频类型')
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse('reverse_path'),result['msg']))

        result=check_and_video_type(FromType,from_to,'非法的来源')
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse('reverse_path'),result['msg']))

        result=check_and_video_type(NationalityType,nationality,'非法的国籍')
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse('reverse_path'),result['msg']))
        if not video_id:
            try:
    ...
    def post(self,request,video_id):
        ...
        if not videosub_id:
            try:
                ...
                return redirect('{}?error={}'.format(reverse(url_format,'创建失败')))
        else:
            ...
            try:
                ...
                return redirect('{}?error={}'.format(reverse(url_format,'修改失败')))
        return redirect(reverse('video_sub',kwargs={'video_id':video_id}))
...
class VideoStartView(View):
    def post(self,request):
        ...
        if not all([name,identity,video_id]):
            return redirect('{}?errror={}'.format(path_format,'缺少必要字段'))
        result = check_and_video_type(IdentityType, identity, '非法的身份')

        if result.get('code') !=0:
            return redirect('{}?error={}'.format(path_format,result['msg']))
        video=Video.objects.get(pk=video_id)
        try:
            VideoStart.objects.create(
                video=video,
                name=name,
                identity=identity
            )
...
class StarDelete(View):
    def get(self,request,star_id,video_id):
        VideoStart.objects.filter(id=star_id).delete()
        return redirect(reverse('video_sub',kwargs={'video_id':video_id}))
